...
class Profile:
    ...
    def __init__(self):
        ...
        """
        self.PHOTO, self.NAME, self.AGE, self.GENDER, self.ABOUT, self.LOCATION = range(
            6
        )
        (
            self.EDIT_NAME,
    ...
    def get_profile_create_conversation_handler(self):
        ...
        for lan in languages:
            ...
            lan_handler = [
                MessageHandler(
                    filters.Regex(rf"^{messages.COMPLETE_PROFILE_BUTTON}$"),
                    self.start_profile,
                ),
            ]
        ...
        return ConversationHandler(
            ...
            states={
                ...
                self.AGE: [
                    MessageHandler(filters.TEXT & ~filters.COMMAND, self.handle_age)
                ],
                self.GENDER: [
                    MessageHandler(
                        filters.Regex("^(Male|Female|Other)$"), self.handle_gender
                    )
                ],
    ...
    def direct_msg_conversation_handler(self):
        ...
        return ConversationHandler(
            entry_points=[
                CallbackQueryHandler(
                    self.start_msg_request, pattern=f"^{self.msg_request_pattern}"
                )
            ],
            states={
    ...
    async def handle_age(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            if age < 13 or age > 120:
                await update.message.reply_text(messages.PROFILE_INVALID_AGE)
                return self.AGE  # Stay in AGE state
            context.user_data["age"] = age
        except ValueError:  # If input is not a valid number
            await update.message.reply_text(messages.PROFILE_AGE_NOT_NUMBER)
            return self.AGE  # Stay in AGE state
        ...
        return [
            [
                InlineKeyboardButton(
                    ...
                    callback_data=f"{self.my_profile_key_starter}: edit",
                ),
                InlineKeyboardButton(
                    messages.EDIT_PHOTO_BUTTON,
                    callback_data=f"{messages.PROFILE_EDIT_PATTERN}: {messages.PHOTO_PATTERN}",
                ),
            ],
            [
                InlineKeyboardButton(
                    f"( {num_likes} ) {messages.LIKES_SHOW_BUTTON}",
                    callback_data=f"{self.rel.rel_inspect_pattern}: {self.rel.like_pattern}",
                ),
                InlineKeyboardButton(
                    f"( {num_friends} ) {messages.FRIENDS_SHOW_BUTTON}",
                    callback_data=f"{self.rel.rel_inspect_pattern}: {self.rel.friend_pattern}",
                ),
            ],
        ]
    ...
    async def show_my_profile(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        generated_id_display = (
            profile.generated_id
        )  # Assuming this is always set if profile exists
        ...
        if profile.profile_photo:
            await update.message.reply_photo(
                photo=profile.profile_photo, caption=text, reply_markup=reply_markup
            )
        else:
            await update.message.reply_text(text, reply_markup=reply_markup)

    async def show_my_profile_edit_mode(
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    messages.EDIT_NAME_BUTTON,
                    callback_data=f"{messages.PROFILE_EDIT_PATTERN}:{messages.NAME_PATTERN}",
                ),
                InlineKeyboardButton(
                    messages.EDIT_ABOUT_BUTTON,
                    callback_data=f"{messages.PROFILE_EDIT_PATTERN}:{messages.ABOUT_PATTERN}",
                ),
                # Mislabeled, should be specific action
            ],
            [
                InlineKeyboardButton(
                    messages.EDIT_CITY_BUTTON,
                    callback_data=f"{messages.PROFILE_EDIT_PATTERN}:{messages.CITY_PATTERN}",
                ),
                InlineKeyboardButton(
                    messages.EDIT_LOCATION_BUTTON,
                    callback_data=f"{messages.PROFILE_EDIT_PATTERN}:{messages.LOCATION_PATTER}",
                ),
            ],
            [
                InlineKeyboardButton(
                    messages.EDIT_LANGUAGE_BUTTON,
                    callback_data=f"{messages.LANGUAGE_PATTERN}",
                ),
            ],
        ]
    ...
    def _target_profile_keyboard(self, user_id, target_id):
        ...
        return [
            [
                InlineKeyboardButton(
                    "Direct MSG",
                    callback_data=f"{self.msg_request_pattern}: {target_id}",
                ),
                InlineKeyboardButton(
                    "Chat Request",
            ...
            [
                (
                    InlineKeyboardButton(
                        "Like",
                        callback_data=f"{self.rel.rel_starter_pattern}:{self.rel.like_pattern}:{target_id}",
                    )
                    if not self.rel_db.is_liked(user_id=user_id, target_id=target_id)
                    else InlineKeyboardButton(
                        "Liked <3",
                        callback_data=f"{self.rel.rel_starter_pattern}:{self.rel.unlike_pattern}:{target_id}",
                    )
                ),
                (
                    InlineKeyboardButton(
                    ...
                    if not self.rel_db.is_friend(user_id=user_id, target_id=target_id)
                    else InlineKeyboardButton(
                        "Added as Friend",
                        callback_data=f"{self.rel.rel_starter_pattern}:{self.rel.unfriend_pattern}:{target_id}",
                    )
                ),
            ],
            [
                (
                    InlineKeyboardButton(
                        "block",
                        callback_data=f"{self.rel.rel_starter_pattern}:{self.rel.block_pattern}:{target_id}",
                    )
                    if not self.rel_db.is_block(user_id=user_id, target_id=target_id)
                    else InlineKeyboardButton(
                        "unblock",
                        callback_data=f"{self.rel.rel_starter_pattern}:{self.rel.unblock_pattern}:{target_id}",
                    )
                ),
                (
                    InlineKeyboardButton(
                        "Report",
                        callback_data=f"{self.rel.rel_starter_pattern}:{self.rel.report_pattern}:{target_id}",
                    )
                    if not self.rel_db.is_report(user_id=user_id, target_id=target_id)
                    else InlineKeyboardButton(
                        "Reported",
                        callback_data=f"{self.rel.rel_starter_pattern}:{self.rel.report_pattern}:{target_id}",
                    )
    ...
    async def show_target_profile(
        ...
        profile = self.user_db.get_user_information(
            target_id
        )  # Retrieve target user's profile from DB
        user_id = update.effective_user.id
    ...
    def load_profile(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        )  # Ensures user exists and updates last_online
        self.user_db.get_user_data(
            update.effective_user.id, context.user_data
        )  # Populates context.user_data
    ...
    def _language_options_keyboard(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE
    ):
    ...
    async def handle_edit_button_click(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE
    ):
        messages = msg(language=context.user_data["lan"])
        query = update.callback_query
    ...
    async def handle_edit_about_input(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE
    ):
    ...
    async def handle_edit_city_input(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE
    ):
        messages = msg(language=context.user_data["lan"])
    ...
    async def handle_edit_location_input(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE
    ):
        messages = msg(language=context.user_data["lan"])
    ...
    async def handle_edit_photo_input(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE
    ):
        messages = msg(language=context.user_data["lan"])
        if not update.message.photo:
    ...
    def get_profile_edit_conversation_handler(self):
        ...
        return ConversationHandler(
            ...
            states={
                self.EDIT_NAME: [
                    MessageHandler(
                        filters.TEXT & ~filters.COMMAND, self.handle_edit_name_input
                    )
                ],
                self.EDIT_ABOUT: [
                    MessageHandler(
                        filters.TEXT & ~filters.COMMAND, self.handle_edit_about_input
                    )
                ],
                self.EDIT_CITY: [
                    MessageHandler(
                        filters.TEXT & ~filters.COMMAND, self.handle_edit_city_input
                    )
                ],
                self.EDIT_PHOTO: [
                    MessageHandler(filters.PHOTO, self.handle_edit_photo_input)
                ],
                self.EDIT_LOCATION: [
                    MessageHandler(filters.LOCATION, self.handle_edit_location_input)
                ],
            },
    ...
    async def start_msg_request(
        ...
        query = update.callback_query
        user_id = update.effective_user.id
        target_id = query.data.split(":")[
            1
        ].strip()  # Extract target_id from callback data
        ...
        )  # Ensure current user_id is in context
        ...
        if chat_db.add_requested_msg(user_id, target_id, the_msg):
            ...
            keyboard = [
                [
                    InlineKeyboardButton(
                        "Accept",
                        callback_data=f"{self.msg_req_command}: accept: {user_id}",
                    ),
                    InlineKeyboardButton(
                        "Decline",
                        callback_data=f"{self.msg_req_command}: decline: {user_id}",
                    ),
                ]
            ]
            reply_markup = InlineKeyboardMarkup(keyboard)
    ...
    @track_user_interaction
    async def chat_request_accepted(
        ...
        if msgs:
            await query.edit_message_text(
                messages.DIRECT_MSG_ACCEPTED.format(target_id=target_id)
            )  # Edit original button message
            for the_msg in msgs:
                await context.bot.send_message(
    ...
    async def chat_reqeust_declined(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE, target_id: int
    ):
        """
        ...
        if message_db.clear_msg_requests_from_map(user_id, target_id):
            await query.edit_message_text(
                messages.DIRECT_MSG_DECLINED.format(target_id=target_id)
            )  # Edit original button message
            await context.bot.send_message(
                target_id,
                text=messages.DIRECT_MSG_DECLINE_NOTIFY.format(user_id=user_id),
            )  # Notify sender
        else:
            await query.edit_message_text(
                messages.DIRECT_MSG_FAILED_DECLINE.format(target_id=target_id)
            )  # Error or no messages

    @track_user_interaction
    async def show_profile_request(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE
    ):
        """
        ...
        if len(target_id_parts) < 2:
            await update.message.reply_text(
                messages.INVALID_FORMAT.format(command="chaT")
            )
            return
    ...
    def show_profile_handler(self):
        ...
        for lan in languages:
            ...
            lan_handler = [
                MessageHandler(
                    filters.Regex(rf"^{messages.PROFILE_BUTTON}$"), self.show_my_profile
                ),
                MessageHandler(
                    filters.Regex(messages.CHAT_PROFILE_REGEX),
                    self.show_profile_request,
                ),
                MessageHandler(
                    filters.Regex(rf"^{messages.EDIT_LANGUAGE_BUTTON}$"),
