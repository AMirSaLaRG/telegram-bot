...
class TorobInteract:
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    messages.CHECK_ITEMS_BUTTON, callback_data="torob: check"
                )
            ],
            [
                InlineKeyboardButton(
                    messages.ADD_NEW_ITEMS_BUTTON,
                    callback_data=self.torob_conversation.query_add_pattern,
                )
            ],
        ]
    ...
    async def edit_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if len(item_id_parts) < 2:
            await update.message.reply_text(
                messages.INVALID_FORMAT.format(command="item")
            )
            return
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    messages.TOROB_EDIT_PRICE,
                    callback_data=f"{self.torob_conversation.edit_price_pattern}",
                ),
                InlineKeyboardButton(
                    messages.TOROB_EDIT_URL,
                    callback_data=f"{self.torob_conversation.edit_url_pattern}",
                ),
                InlineKeyboardButton(
                    messages.TOROB_EDIT_NAME,
                    callback_data=f"{self.torob_conversation.edit_name_pattern}",
                ),
            ],
            [
                InlineKeyboardButton(
                    messages.TOROB_EDIT_DELETE,
                    callback_data=f"{self.torob_conversation.delete_item_pattern}",
                )
            ],
        ]
    ...
    async def check_torob_list(self, query, context: ContextTypes.DEFAULT_TYPE):
        ...
        if user_items:
            for item in user_items:
                ...
                signal_price = (
                    messages.PRICE_OK_ICON
                    if latest_price and latest_price <= item.user_preferred_price
                    else messages.PRICE_HIGH_ICON
                )
        ...
        if final_note:
            await query.edit_message_text(text=final_note, reply_markup=reply_markup)
        else:
            await query.edit_message_text(
                text=messages.NOTHING_TO_SHOW, reply_markup=reply_markup
            )
    ...
    def handlers(self):
        ...
        for lan in languages:
            ...
            lan_handlers = [
                MessageHandler(
                    filters.Regex(rf"^{messages.TOROB_BUTTON}$"), self.torob
                ),
                MessageHandler(
                    filters.Regex(messages.ITEM_EDIT_REGEX), self.edit_command
                ),
            ]
