...
from telegram import (InlineKeyboardButton, InlineKeyboardMarkup, InlineQueryResultArticle,
                      InputTextMessageContent, InlineQueryResultPhoto, InputMediaPhoto, Update)
...
class ShowCases:
    ...
    async def inline_show_selected_users(self, update: Update, context: ContextTypes.DEFAULT_TYPE, selected_users: list[dict[str, float | int | bool | User]], max_num:int=50):
        """
    ...
    async def show_selected_users(self, query, context:ContextTypes.DEFAULT_TYPE, selected_users=None, current_page=1, title=""):
        """
        ...
        keyboard_buttons = [
            InlineKeyboardButton('look Inline mode', switch_inline_query_current_chat =messages.QUERY_PATTERN_FILTERED_PPL)
        ]
        if total_pages > 1:
            if current_page > 1:
    ...
    async def buttons(self, update, context):
        ...
        if query.data == 'page_before':
            await self.show_selected_users(query, context, current_page=context.user_data.get('current_page', 1) - 1)
